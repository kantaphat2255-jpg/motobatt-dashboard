...
import type { ZoneBreakdownRow } from '@/lib/types';
...
export default function ZoneBarChart({ zones }: Props) {
  const data = zones.map(z => ({
    เขต: z.zoneId,
    ยอดขาย: Math.round(z.sales),
  }));
  ...
  return (
    <ResponsiveContainer width="100%" height={220}>
      <BarChart data={data} margin={{ top: 4, right: 8, left: 8, bottom: 4 }}>
        ...
        <XAxis dataKey="เขต" tick={{ fill: '#9CA3AF', fontSize: 12 }} axisLine={false} tickLine={false} />
        ...
        />
        <Bar dataKey="ยอดขาย" radius={[4, 4, 0, 0]} fill="#F5C400" />
      </BarChart>
    </ResponsiveContainer>
  );
}
